...
export default function ServiceTypesScreen() {
  ...
  const stringifiedServices = JSON.stringify(values.services);

  const enabledTransportTypes = useMemo(() => {
    if (!transportId || !stringifiedServices) return false;
    return values.services.some(
      (service) => service.id === transportId && service.isActive
    );
  }, [values.services, stringifiedServices, transportId]);

  // Clear transportIds when transportation service is toggled off (side effect, not during render)
  useEffect(() => {
    if (!enabledTransportTypes) {
      form.setValue("transportIds", []);
    }
  }, [enabledTransportTypes, form]);
